...
6. Set `review_max_steps` (2-32; default 8) for review/rereview, independently of worker steps. Set role output authorizations `implementation_max_output_tokens` and `review_max_output_tokens` (positive integers; default 16000). Before authorization, freeze every primary/fallback model's trusted output capability and source/version in `model_capabilities`, including any API model alias. Context is separate and may be unknown. Missing capability blocks authorization; effective output is the smaller of frozen capability and role authorization. Old plans normalized with new fields require a new hash approval; never update old run records automatically.
7. Run `agentflow --project <root> plan show` and present the exact SHA-256 plus the effective scope, models, file boundaries, remote budget, privacy policy, mode, and expiry.
8. Wait for explicit approval of that plan hash. Only then run `agentflow --project <root> plan authorize --hash <sha256>` and `agentflow --project <root> start <plan-id>`.
...
- Use the installed adapter's compatibility checks. A supported patch upgrade may proceed after runtime checks pass; do not bypass a failed permission, endpoint, transport, or output-budget check. A model listed in configuration may not be installed or loaded; verify the local runtime before selecting it.
